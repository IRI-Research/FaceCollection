...
namespace NIUI
{
	...
	NIUI_STATUS Core::Initilise(const char* xmlPath, int maxUsers)
	{
	...
	bool Core::GetJointLocation(int userID, int joint, UDKVector3* vec)
	{
	...
	bool Core::GetUserCentre(int userID, UDKVector3* vec)
	{
		int index = GetUserIndex(userID);
	...
	bool Core::GetLocalJointLocation(int userID, int joint, UDKVector3* vec)
	{
	...
	bool Core::GetBoneAxes(int userID, int joint, UDKVector3* forward, UDKVector3* side, UDKVector3* up)
	{
		if (joint >= NIUI_JOINT_COUNT)
		{
			return false;
